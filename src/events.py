    ...
    def event_queue(player,map):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                player.player.move_player_last = False
                Events.destination_x = Events.mouse_x
                Events.destination_y = Events.mouse_y
                player.player.move_player_live = True
            if event.type == pygame.MOUSEBUTTONUP:
                player.player.move_player_live = False
                Events.destination_x = Events.mouse_x
                Events.destination_y = Events.mouse_y
                player.player.move_player_last = True
            ...
            if event.type == pygame.QUIT:
                exit()
        if player.player.move_player_live:
            player.player.move_sprite((Events.mouse_x-25,Events.mouse_y-35))
        if player.player.move_player_last:
